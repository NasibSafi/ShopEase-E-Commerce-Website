...
class OrderCommitView(View):
    ...
    def post(self,request):
        ...
        if not request.user.is_authenticated:
            return JsonResponse({"res":0,"msg":"Login timed out, please log in again"})
        ...
        if pay_method not in OrderInfo.PAY_METHOD:
            return JsonResponse({"res":0,"msg":"The payment method is not in compliance"})
        try:
           address = Address.objects.get(id=addr_id)
        except Address.DoesNotExist:
            return JsonResponse({"res":0,"msg":"The delivery address is wrong"})
            ...
            for sku_id in sku_ids:
                for i in range(3):
                    try:
                        ...
                        return JsonResponse({"res":0,"msg":"no related products"})
                    ...
                    if sku.stock < count:
                        transaction.savepoint_rollback(sid)
                        return JsonResponse({"res":0,"msg":"Out of stock"})
                    ...
                    if res == 0:
                        if i == 2:
                            transaction.savepoint_rollback(sid)
                            return JsonResponse({"res":0,"msg":"Order failed"})
                        continue
        ...
        return JsonResponse({"res":1,"msg":"Submit the order successfully"})
...
class OrderPayView(View):
    """Call the Alipay payment interface: verify that the front-end passes the product order number"""
    def post(self,request):
        ...
        try:
            ...
            return JsonResponse({"res":0,"msg":"no order information"})
        alipay = AliPay()
